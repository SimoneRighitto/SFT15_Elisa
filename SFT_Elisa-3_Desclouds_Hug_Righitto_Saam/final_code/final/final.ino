...
static unsigned long int startChangeColor = getTime100MicroSec();
  ...
  if(batteryLevel < DESIRED_BATTERY_LEVEL ) {
      ...
      if((getTime100MicroSec() -startChangeColor) >= (PAUSE_2_SEC)) {
          
          
               startChangeColor = getTime100MicroSec();
...
int checkMode(){
  ...
  if (pwm_red==0 && pwm_green == 255 && pwm_blue == 255){
    ...
    //base mode
   else if(pwm_red==0 && pwm_green == 0 && pwm_blue == 255){
     return 1;
     }
